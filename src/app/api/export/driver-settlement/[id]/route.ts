...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const year = (settlement.paidOn ?? settlement.periodEnd).slice(0, 4);
  const ytdPaid = dataset.driverSettlements
    .filter((row) => row.driverId === settlement.driverId && row.status === "PAID" && row.paidOn?.startsWith(year))
  ...
  rows.push([]);
  rows.push(["PAY SUMMARY", "", "", "", "", "", "", "", "", "", "", ""]);
  rows.push(["Base load pay", "", "", "", "", "", "", "", "", totals.basePay, "", ""]);
  for (const adjustment of settlement.adjustments) {
    const definition = DRIVER_ADJUSTMENT_TYPES.find((row) => row.id === adjustment.type)!;
    const amount = definition.direction === "SUBTRACT" ? -adjustment.amount : adjustment.amount;
    rows.push([definition.label, "", adjustment.reason, "", "", "", "", "", "", amount, "", ""]);
  }
  rows.push(["Net pay", "", "", "", "", "", "", "", "", totals.netPay, settlement.status, settlement.paidOn ?? ""]);
  rows.push([`${year} YTD paid`, "", "", "", "", "", "", "", "", ytdPaid, "", ""]);
  const csv = toCsv({
    title: "Driver statement",
    columns: ["Pickup date / item", "Load number", "Route / reason", "Truck", "Pay method", "Pay rate", "Gross hauled", "Loaded miles", "Total miles", "Pay amount", "Status", "Paid on"],
    rows,
  });
