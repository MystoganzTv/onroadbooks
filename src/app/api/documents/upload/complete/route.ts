...
import { finishChunkedUpload } from "@/lib/storage/chunked-upload";
...
const completionSchema = z
  .object({ ticket: z.string().min(20).max(16_384) })
  .strict();
...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return response({ error: "The upload confirmation is invalid." }, 400);
  ...
  const existing = dataset.documents.find(
  ...
  if (!storage.info)
    return response({ error: "Direct document storage is unavailable." }, 409);

  try {
    if (storage.chunkedUploads) await finishChunkedUpload(storage, ticket);
    const stored = await storage.info(ticket.storageKey);
    if (!stored)
      return response({ error: "The uploaded document was not found." }, 404);
    if (
      stored.sizeBytes !== ticket.sizeBytes ||
      stored.contentType !== ticket.contentType
    ) {
      await storage.remove(ticket.storageKey);
      return response(
        { error: "The stored document did not match the authorized upload." },
        400,
      );
    }
