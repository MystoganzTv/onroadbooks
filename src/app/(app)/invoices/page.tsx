...
import { invoiceAgeDays, invoiceIsOverdue, nextInvoiceNumber } from "@/lib/invoices";
...
export default async function InvoicesPage() {
  ...
  const overdue = outstanding.filter((load) => invoiceIsOverdue(load));
  ...
  return (
    <div className="space-y-4 p-4 lg:p-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <MiniStat label="Outstanding" value={formatMoney(outstanding.reduce((sum, load) => sum + load.grossRate, 0))} sub={`${outstanding.length} invoices`} />
        <MiniStat label="Overdue" value={formatMoney(overdue.reduce((sum, load) => sum + load.grossRate, 0))} sub={`${overdue.length} invoices`} tone={overdue.length ? "warning" : undefined} />
        <MiniStat label="Collected" value={formatMoney(paid.reduce((sum, load) => sum + load.grossRate, 0))} sub={`${paid.length} invoices`} />
